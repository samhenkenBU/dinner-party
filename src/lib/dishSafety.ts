...
export type SafetyRating = "green" | "yellow" | "red" | "unknown";
...
export function getSaferVariants(
  ...
  return dish.variants
    .map((v): { label: string; rating: SafetyRating } => {
      ...
      const rating: SafetyRating = affectedGuests >= 2 ? "red" : affectedGuests === 1 ? "yellow" : "green";
